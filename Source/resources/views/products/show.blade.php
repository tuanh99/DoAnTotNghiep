...
document.addEventListener('DOMContentLoaded', function() {
    let numProductInput = document.querySelector('.num-product');
    let stock = {{ $product->stock }};
    let form = document.querySelector('.add-to-cart-form');

    form.addEventListener('submit', function(event) {
        let numProduct = parseInt(numProductInput.value);

        if (numProduct > stock) {
            event.preventDefault(); // Ngăn chặn form submit

            alert(`Chỉ còn ${stock} sản phẩm. Vui lòng chọn số lượng nhỏ hơn hoặc bằng.`);
        }
    });

    // Thay đổi thông báo mặc định của browser
    numProductInput.addEventListener('input', function() {
        let numProduct = parseInt(numProductInput.value);

        if (numProduct > stock) {
            numProductInput.setCustomValidity(`Số lượng sản phẩm chỉ còn ${stock}. Vui lòng chọn số lượng nhỏ hơn hoặc bằng.`);
        } else {
            numProductInput.setCustomValidity('');
        }
    });
});
</script> -->
